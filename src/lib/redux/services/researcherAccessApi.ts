...
/**
 * Researcher access to a company's programs.
 *
 * Approval is granted per *organization*, so one record covers every program
 * that company runs — which is why the researcher-facing screens key on the
 * organization and only the pre-check is asked per program.
 *
 * Every mutation invalidates `ResearcherAccess`, and the report form's
 * pre-check provides it: approving someone in the company queue unblocks their
 * submit button without either side reloading.
 */
...
export interface ResearcherAccessList {
  ...
  /** Across the whole queue, not just this page. */
  total: number;
  page: number;
  totalPages: number;
}
...
/** The upstream refuses anything larger. */
...
export const researcherAccessApi = proxyApi.injectEndpoints({
  endpoints: (builder) => ({
    /**
     * Whether the caller may file against this program — asked before the
     * form offers a submit button rather than discovered by a 403 afterwards.
     */
    getReportingAccess: builder.query<ProgramReportingAccess, string>({
      ...
      providesTags: (result, _error, programId) => [
        ...
        /* Also keyed by organization: approval is company-wide, so a decision
           taken anywhere invalidates the pre-check for every program of that
           company the reporter has open. */
    ...
    /** The caller's own record with one company; `null` when never asked. */
    ...
    /** Every company the caller has approached — the "My access" screen. */
    ...
    /** The researcher asks. 409 when one is already pending or approved. */
    ...
    /** The company's review queue, oldest first. */
    ...
    >({
      query: ({ organizationId, status, page = 0, size = RESEARCHER_PAGE_SIZE }) => {
        const params = new URLSearchParams({
          page: String(Math.max(0, page)),
          size: String(size),
          /* Oldest first: a queue is worked from the front, and the person who
             has been waiting longest is the one to answer next. */
    ...
    /** Pre-approval, by user id — the account has to exist already. */
    ...
    /** Approve, reject or revoke. Illegal transitions come back as 409. */
