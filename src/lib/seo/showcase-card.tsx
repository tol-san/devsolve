...
export async function showcaseCard(showcase: ShowcaseResponse | null) {
  const font = await readFile(join(process.cwd(), "node_modules/next/dist/compiled/@vercel/og/Geist-Regular.ttf"));
  const title = clip(showcase?.title?.trim() || "Showcase", 100);
  const description = clip(describe(showcase?.overview, "", 240), 135);
  const author = showcaseAuthor(showcase);
  const category = clip(showcase?.categoryName?.trim() || "Showcase", 30);
