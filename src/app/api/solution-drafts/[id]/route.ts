...
export async function GET(
  ...
  try {
    const upstream = await upstreamFetch(`/solution-drafts/${id}`, token);
    return relay(upstream, "Unable to load solution draft.");
  } catch {
    return unreachable();
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    return relay(upstream, "The solution draft could not be discarded.");
  } catch {
    return unreachable();
  }
}
