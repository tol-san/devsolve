...
// Compact Member Card that displays the FULL photo without aggressive cropping
function MemberCard({
  member,
  badgeLabel,
  roleLabel,
  ...
  return (
    ...
    >
      {/* Compact Photo Container: aspectRatio 6/7 shows the full person without cutting in half */}
      <div
      ...
      >
        ...
        {/* Minimal bottom gradient overlay for badge readability without darkening the photo */}
        <div className="absolute inset-x-0 bottom-0 h-10 bg-linear-to-t from-slate-950/35 to-transparent pointer-events-none" />

        {/* Role Pill */}
        <span className="absolute bottom-2.5 left-2.5 rounded-lg bg-white/95 px-2.5 py-1 text-[11px] font-bold uppercase tracking-[0.1em] text-slate-700 backdrop-blur-md shadow-xs dark:bg-neutral-900/90 dark:text-neutral-200 border border-white/20">
          {roleLabel}
        </span>
      </div>
    ...
    >
      <div className="relative mx-auto w-full max-w-5xl px-6 sm:px-10">
        ...
        <div className="grid grid-cols-1 items-start justify-items-center gap-6 sm:grid-cols-2 sm:gap-7 lg:grid-cols-3">
          {/* Team Intro Block */}
          <div className="team-intro-block flex h-full w-full max-w-[285px] sm:max-w-[300px] flex-col justify-center py-2 sm:py-4">
            <div>
          ...
          {SUPERVISORS.map((mentor) => (
            <div
              key={mentor.name}
              className="w-full max-w-[285px] sm:max-w-[300px]"
            >
              <MemberCard
                member={mentor}
