...
8. **One Shared Page Background**: every page in the app sits on the landing hero's backdrop — grid paper, drifting colour fields, pulsing cells, scan beams and rising motes. It is mounted once per shell by `PageBackdrop` (`src/components/layout/PageBackdrop.tsx`, a `fixed inset-0 -z-10` wrapper around `SectionBackdrop`) in the `(public)`, `(auth)` and `dashboard` layouts, plus `NotFoundStage` for the 404, which renders outside those layouts.
